...
class JacocoParserTest {
    ...
    void shouldConvertCodingStyleToTree() {
        ...
        assertThat(tree.getMetricsDistribution()).containsExactly(
                ...
                entry(FILE, new Coverage(FILE, 7, 3)),
                entry(CLASS, new Coverage(CLASS, 15, 3)),
                entry(METHOD, new Coverage(METHOD, 97, 5)),
                entry(INSTRUCTION, new Coverage(INSTRUCTION, 1260, 90)),
                entry(LINE, new Coverage(LINE, 294, 29)),
                entry(BRANCH, new Coverage(BRANCH, 109, 7)),
        ...
        Node any = tree.getAll(FILE).stream().filter(n -> n.getName().equals("Ensure.java")).findAny().orElseThrow(() -> new NoSuchElementException("Blub"));
    ...
    void shouldSplitPackages() {
        ...
        assertThat(tree.getMetricsDistribution()).contains(
                entry(PACKAGE, new Coverage(PACKAGE, 4, 0)));
